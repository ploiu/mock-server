...
const { route } = defineProps<RouteListEntryProps>();

// copy of the route so that we don't modify the original
const emit = defineEmits<{
    (e: "change", value: UIRoute);
    (e: "delete", value: UIRoute);
    (e: "select", value: UIRoute);
}>();

const toggleEnabled = (e: ChangeEvent) => {
    const updatedRoute = { ...route, isEnabled: e.target.checked };
    emit("change", updatedRoute);
};
</script>

<template>
    <Card class="route-list-entry" @click="emit('select', route)">
        <template #title>
            <span
                :class="[
                    'request-method',
                ...
                >{{ route.method }}</span
            >
            <span class="route-title">{{ route.title }}</span></template
        >
        <template #footer>
            <div class="row">
